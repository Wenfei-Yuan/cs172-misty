...
// Thorough reading phase with end-of-session check-in screen.
// No inline interruptions — questions appear only when exiting the reader.
...
(function () {
  ...
  pill.id = "phase-pill";
  pill.innerHTML = `<span id="phase-pill-label">Thorough</span>`;

  const toolbarDivider = shadow.querySelector(".toolbar-divider");
  if (toolbarDivider) toolbarDivider.after(pill);
  else shadow.getElementById("reader-toolbar")?.appendChild(pill);
  ...
  function setReadingMode(mode) {
    currentPhase = (mode === "full" || mode === "para") ? "thorough" : null;
    syncPhaseState();
    clearLegacyDimming();
    if (mode === "full") {
      enterThoroughPhase({ syncFullMode: false });
    } else if (mode === "para") {
  ...
  // greyed out at paragraph granularity.
  // Navigate with ↑/↓ arrow keys or by scrolling.

  function enterThoroughPhase({ syncFullMode = true } = {}) {
    currentPhase = "thorough";
    syncPhaseState();
    clearLegacyDimming();
    removeThoroughFocus();
